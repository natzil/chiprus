(function(){
  ...
  function renderProjects(){
    ...
    const card = p => { const current = projectXp(p); const pct = p.target_xp ? current / p.target_xp * 100 : 0; return `<div class="project-card"><div class="project-top"><div><div class="project-name">${esc(p.name)}</div><div class="project-tags">важность: ${esc(p.importance)} · сложность: ${esc(p.difficulty)}</div></div><div class="project-xp">${current} / ${Number(p.target_xp || 0)} XP</div></div><div class="seg-bar" style="--seg-color:#c9895b" data-percent="${pct}"></div><div class="project-stage">этап: <b>${esc(p.current_stage || 'следующий шаг')}</b></div></div>`; };
    const chip = p => { const current = projectXp(p); const pct = p.target_xp ? current / p.target_xp * 100 : 0; return `<div class="m-project-chip"><div class="name">${esc(p.name)}</div><div class="seg-bar" style="--seg-color:#c9895b" data-percent="${pct}"></div><div class="meta">${current}/${Number(p.target_xp || 0)} XP · ${esc(p.current_stage || '')}</div></div>`; };
  ...
  function renderAreas(){
    ...
    const ccnaPct = lessons.length ? done / lessons.length * 100 : 0;
    const english = englishStats();
    const hobbyDone = sum((state().projects || []).filter(p => p.status === 'active'), p => projectXp(p));
    const hobbyTarget = sum((state().projects || []).filter(p => p.status === 'active'), p => p.target_xp);
    const hobbyPct = hobbyTarget ? hobbyDone / hobbyTarget * 100 : 0;
    const areas = [
      { name:'Обучение', level:'Lv.' + levelFor(stats().total), accent:'#6ee7da', pct:(ccnaPct + english.percent) / 2, subs:[['CCNA', ccnaPct], ['English', english.percent], ['Linux', 0]] },
      { name:'Хобби / проекты', level:'Lv.' + Math.max(1, Math.floor(hobbyPct / 25) + 1), accent:'#c9895b', pct:hobbyPct, subs:[['NAS', projectPercent('NAS')], ['ESP32', projectPercent('ESP32')]] }
    ];
  ...
  function renderAdminState(){
    const isAuthed = !!state().user; const demo = !!state().demoMode;
    document.querySelectorAll('.admin-only').forEach(el => { el.style.display = (isAuthed || demo) ? '' : 'none'; });
    const html = isAuthed ? `<span class="email">${esc(state().user.email || 'demo user')}</span><button class="mini-btn" data-auth-logout>Выйти</button>` : `<input data-auth-email placeholder="email"><input data-auth-password type="password" placeholder="password"><button class="mini-btn" data-auth-login>Войти</button>`;
    ...
    const panel = document.getElementById('admin-panel'); if (panel) panel.innerHTML = `<div class="area-title">Admin</div><div class="project-stage">${demo ? 'Demo/offline mode: изменения локальные.' : isAuthed ? 'Можно редактировать Supabase.' : 'Войди для редактирования.'}</div>`;
  ...
  function renderDashboard(){ renderCharacter(); renderQuests(); renderProjects(); renderAreas(); renderEnglishProgress(); renderCcnaProgress(); renderAdminState(); renderAllSegments(); }
