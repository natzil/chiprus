...
insert into projects(name, description, area, status, importance, difficulty, target_xp, current_stage) values
  ('NAS / домашний сервер', 'Домашнее хранилище, сервисы и резервные копии', 'hobby', 'active', 'высокая', 'средняя', 500, 'выбрать ОС для сервера'),
  ('ESP32 + RFID замок', 'Pet-проект контроля доступа на ESP32', 'hobby', 'active', 'средняя', 'средняя', 400, 'прошивка чтения карт')
...
on conflict (course, day_number) do nothing;
